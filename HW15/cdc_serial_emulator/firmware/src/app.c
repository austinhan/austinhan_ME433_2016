            ...
            if(appData.isReadComplete == true)
            {
                ...
                for(ii=0; ii<appData.readLength; ii++) {
                    if (appData.readBuffer[ii] == '\n' || appData.readBuffer[ii] == '\r'){
                        ...
                        int full = 5000;
                        int k = (full-500)/(325^2);
                        if (qq <= 320) {
                            OC2RS = full - 500 - k/10*((320-qq)^2);; // left full speed
                            OC1RS = full - 500 - k*((320-qq)^2); // right speed decreases w/ increasing deviation
                        } else if (qq == 641) { // failsafe if robot loses line
                            OC1RS = OC1RS;
                            OC2RS = OC2RS;
                        } else {
                            OC1RS = full - 500 - k/10*((qq-320)^2); // right full speed
                            OC2RS = full - 500 - k*((qq-320)^2); // left speed decreases w/ increasing deviation;
                        }
//                        if (qq < 320) {
//                            OC2RS = full; // left full speed
//                            OC1RS = full - (320-qq)/320; // right speed decreases w/ increasing deviation
//                        } else if (qq == 641) { // failsafe if robot loses line
//                            OC1RS = OC1RS;
//                            OC2RS = OC2RS;
//                        } else if (qq == 320) {
...
//                            OC2RS = full;
//                        } else {
//                            OC1RS = full; // right full speed
//                            OC2RS = full - (qq - 320)/320; // left speed decreases w/ increasing deviation;
//                        }
